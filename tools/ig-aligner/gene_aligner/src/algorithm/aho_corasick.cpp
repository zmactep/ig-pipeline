...
Node* Node::getLink(char c) const {
  std::map<char, Node *>::const_iterator iter = links.find(c);
...
void AhoCorasick::cleanup() {
  ...
  while (!toDel.empty()) {
    ...
    for (std::map<char, Node *>::const_iterator it = links.begin(); it != links.end(); ++it) {
...
void AhoCorasick::init() {
  ...
  while (!q.empty()) {
    ...
    q.pop();
    for (std::map<char, Node *>::const_iterator iter = current_node->links.begin();
