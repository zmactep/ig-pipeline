...
# At least 2 consequative 4s
def fix_fr2_start(seq, pos):
    if seq[pos] == '4' and seq[pos + 1] == '4':
        return pos
    else:
        return fix_fr2_start(seq, seq.find('4', pos + 1))
...
def fill_gaps(seq):
    ...
    borders[0] = 0
    borders[-1] = len(seq)
    borders[4] = fix_fr2_start(seq, borders[4])
...
def fix(input_str, start_window=3, end_window=17, step_window=2):
...
def main():
    parser = argparse.ArgumentParser(description='Fix prediction')
    parser.add_argument('--input', help='0011122333444555666-like string')
    args = parser.parse_args()
    print(fix(args.input))
