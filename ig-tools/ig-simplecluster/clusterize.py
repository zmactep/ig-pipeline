...
def main():
    ...
    if not os.path.isdir(os.path.abspath(args.out)):
        os.mkdir(os.path.abspath(args.out))

    if args.ctype == 'cluster':
        clustalo.run(args.src, args.out, args.minlen)
    else:
        alignedsim.run(args.src, args.out, args.minlen, args.m, args.useprct, args.cons)
